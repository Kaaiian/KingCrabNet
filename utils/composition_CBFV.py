...
def _assign_features(matrices, elem_info, formulae, n=8):
    ...
    max_composition_size = n
    for h in tqdm.tqdm(range(len(formulae))):
        elem_list = formula_mat[h]
        target = target_mat[h]
        formula = formulae[h]
        count = np.zeros(max_composition_size)
        cut = max_composition_size
        if len(count_mat[h]) < max_composition_size:
            cut = len(count_mat[h])
        count[:cut] = count_mat[h][:cut]
        comp_mat = np.zeros(shape=(max_composition_size, elem_mat.shape[-1]))
        i = 0
        for elem in elem_list:
            if i == max_composition_size:
                break
        ...
        conc_list = [comp_mat, count.reshape(max_composition_size, 1)]
        final_feat = np.concatenate(conc_list, axis=1).ravel()
...
def generate_features(df, elem_prop='onehot', drop_duplicates=True, n=8):
    '''
    Parameters
    ----------
    df: Pandas.DataFrame()
    ...
    elem_props = pd.read_csv(dirpath + '/data/element_properties/'
                             +elem_prop+'.csv')
    elem_props.index = elem_props['element'].values
    elem_props.drop(['element'], inplace=True, axis=1)
    # get the column names
    cols = elem_props.columns.values
    # find the mean value of each column
    median_values = elem_props[cols].median()
    # fill the missing values in each column with the columns mean value
    elem_props[cols] = elem_props[cols].fillna(median_values.iloc[0])
    scaler = StandardScaler()
    # elem_props = pd.DataFrame(normalize(scaler.fit_transform(elem_props)),
    #                           columns=elem_props.columns,
    #                           index=elem_props.index)
    ...
    col_names = []
    for num in range(n):
        col_name = 'elem' + str(num) + '_' + elem_props.columns.values
        col_names += col_name.tolist()
        col_names += ['elem' + str(num) + '_count']
