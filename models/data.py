...
# %%
def get_edm(path, elem_prop='oliynyk', n_elements=8):
    """
    Build a element descriptor matrix.
    ...
    from utils.composition_CBFV import generate_features
    df = pd.read_csv(path)
    if 'formula' not in df.columns.values.tolist():
        df['formula'] = df['cif_id'].str.split('_ICSD').str[0]
    X, y, formula, skipped = generate_features(df, elem_prop, n=n_elements)
    edm_array = X.reshape(X.shape[0], n_elements, -1)
...
class EDMDataLoader():
    ...
    def __init__(self, data_dir, elem_prop='onehot',
        ...
        self.train_data = get_edm(self.train_loc, elem_prop=self.elem_prop)
        self.val_data = get_edm(self.val_loc, elem_prop=self.elem_prop)
        self.test_data = get_edm(self.test_loc, elem_prop=self.elem_prop)
...
class EDM_CsvLoader():
    ...
    def __init__(self, csv_data, csv_val=None, val_frac=0.2, batch_size=64,
                 ...
                 elem_prop='mat2vec', pin_memory=True):
        self.csv_data = csv_data
        self.csv_val = csv_val
        self.val_frac = val_frac
        self.main_data = list(get_edm(self.csv_data, elem_prop=elem_prop))
        ...
        if self.csv_val is None:
            ...
            self.train_data = self.main_data
            self.val_data = list(get_edm(self.csv_val, elem_prop=elem_prop))
